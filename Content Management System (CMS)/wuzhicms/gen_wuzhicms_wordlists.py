#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# File name          : get_wuzhicms_paths.py
...
if __name__ == '__main__':
    ...
    if os.path.exists("./versions/"):
        ...
        os.system('find ./versions/ -type f -name "wuzhicms.txt" -exec cat {} \\; | sort -u > wuzhicms.txt')
        os.system('find ./versions/ -type f -name "wuzhicms_files.txt" -exec cat {} \\; | sort -u > wuzhicms_files.txt')
        os.system('find ./versions/ -type f -name "wuzhicms_dirs.txt" -exec cat {} \\; | sort -u > wuzhicms_dirs.txt')
    
        if not options.no_commit:
            if options.verbose:
                print("      [>] Committing results ...")
                os.system('git add *.txt; git commit -m "Added general wordlists for wuzhicms";')
            else:
                os.popen('git add *.txt; git commit -m "Added general wordlists for wuzhicms";').read()

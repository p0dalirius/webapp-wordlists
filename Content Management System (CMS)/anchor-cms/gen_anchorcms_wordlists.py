...
if __name__ == '__main__':
    ...
    versions = get_releases_from_github("anchorcms", "anchor-cms")
    ...
    for version in versions.keys():
        ...
        if not os.path.exists('./versions/%s/' % (str_version)):
            ...
            print('[>] Ignoring anchorcms version %s (local wordlists exists)' % str_version)

        if generate:
            print('[>] Extracting wordlists for anchorcms version %s' % str_version)
